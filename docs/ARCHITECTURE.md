...
5. **Writes** Hugo-ready country pages (`content/tournaments/countries/{country}.md`) and rewrites **`data/countries.json`** (dump of every country tab). Aggregated stats are also assembled as an in-memory dict for Google Sheets export (`build_output_data()`).
...
| `data/countries.json` | JSON | Public dump of **every country tab** in the spreadsheet (sheet sections). Rewritten from Google Sheets on every save (`-ug`, `-at`, `-et`, `-u`, `-f`) |
...
**Role:** Load/export country worksheets in the v2 schema (Tournaments, Links, Podium, Rosters, Languages, Names, Teams, Players, Errors). Also dumps all public tabs to `data/countries.json`.
...
All have `to_dict()` / `from_dict()` for Sheets round-trips.
...
- `roster_complete`: `yes` / `no` — if `no`, the edition is listed in Errors / **Нет данных** (past tournaments only)
...
- `team id` disambiguates rosters when tied teams share the same `place`
...
| `data/` | Public JSON dump `countries.json` of all country tabs, rewritten whenever stats are saved |
...
| Podium `place` / `id` shows as a date | Cell inherited a date number format | Loader recovers via `parse_sheet_int()`; re-export clears formats (`parse=False`) |
